...
| `NEXT_PUBLIC_SITE_URL` | The deployed site's own URL (e.g. `https://peepandbeyond.vercel.app` or your custom domain), used to build Oreem's `redirect_url`. **Strongly recommended in production**, especially if you use a custom domain — the card-payment route refuses to create a payment session (returns a friendly error) rather than redirect a paying customer to `localhost`. Without it, falls back to `https://$VERCEL_PROJECT_PRODUCTION_URL` (Vercel's stable production domain) and only then to `https://$VERCEL_URL` (a URL unique to that one deployment, which stops working after the next deploy — never rely on this one for real payments). |
...
| `ADMIN_TOOLS_SECRET` | *Optional.* Gates `GET /api/admin/oreem-lookup` — a reconciliation tool for when a payment reaches Oreem but the order never completes on our confirmation page (e.g. a customer never makes it back to the site). Generate the same way as `DIGITAL_ORDER_SECRET`. Without it, the lookup route always returns 401 — nothing is exposed until you set this. Usage: open `https://<your-site>/api/admin/oreem-lookup?reference=<Oreem's AP-... reference from the merchant dashboard>&secret=<this value>` in a browser. |
