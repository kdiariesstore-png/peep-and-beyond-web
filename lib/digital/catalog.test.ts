...
describe("DIGITAL_PRODUCTS", () => {
  it("has exactly 10 products, each with Arabic and English names", () => {
    expect(DIGITAL_PRODUCTS).toHaveLength(10);
  ...
  it("prices the 7 general-topic guides plus the toolkit at 2.990 BHD", () => {
    const flatPriced = DIGITAL_PRODUCTS.filter(
      (p) => p.id !== "activity-book" && p.id !== "peep-storybook"
    );
